...
\begin{itemize}
	...
	\item Solicitações de mudanças no cronograma definido deverão ser realizadas através do formulário de requisição de mudança (ver apêndice \ref{ch:change-request-form}), conforme previsto no plano de gerenciamento das mudanças (ver capítulo \ref{ch:change-management-plan})
	\item A estimativa de tempo para cada atividade deve ser determinada através da opinião especializada, utilizando a técnica de \foreign{Program, Evaluation and Review Technique} (PERT), conforme equação \ref{eq:time-pert}.
